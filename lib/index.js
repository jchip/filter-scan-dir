  ...
  try {
    ...
    for (let ix = 0; !stop && ix < files.length; ix++) {
      ...
      if (options.fullStat) {
        ...
        const fullFile = options._path.join(dir, file.name);
...
function makeOptions(options) {
  ...
  const fastJoin = {
    join: (a, b) => (a && b ? a + Path.posix.sep + b : a ? a : b), // eslint-disable-line
  };
  ...
  opts.readdirOpts = opts.fullStat ? undefined : { withFileTypes: true };
